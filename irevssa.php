...
$num_matches = isset($_POST['num_matches']) ? $_POST['num_matches'] : 0;
?>
...
<div class="container mt-5">
    ...
    <?php else: ?>
        ...
        <form action="save_matches1.php" method="post">
            <input type="hidden" name="num_matches" value="<?= $num_matches ?>">
            <?php for ($i = 1; $i <= $num_matches; $i++): ?>
                <div class="form-group">
                    ...
                    <label for="match_time_<?= $i ?>">Venue:</label>
                    <input type="text" id="match_time_<?= $i ?>" name="match_venue[]" class="form-control" required>
                </div>
            <?php endfor; ?>
